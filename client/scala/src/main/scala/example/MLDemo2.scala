...
object MLDemo2 extends App {
  ...
  try {
    ...
    val pathToData = irisBlob.download(
      componentPath = Some("/Users/nhat/Documents/data/iris"),
      downloadToPath = Some("iris")
    ).get
    val irisDF = sparkSession.read.parquet(f"${pathToData}/iris.parquet")
